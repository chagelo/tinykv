...
func (server *Server) KvGet(_ context.Context, req *kvrpcpb.GetRequest) (*kvrpcpb.GetResponse, error) {
	...
	// 之前的某个事务的给这个 key 上了锁，还没释放
	// 开始时间小于当前事务，所以当前事务读到的应该是之前的结果。
...
func (server *Server) KvPrewrite(_ context.Context, req *kvrpcpb.PrewriteRequest) (*kvrpcpb.PrewriteResponse, error) {
	...
	for _, key := range keys {
		...
		// 有一个写事务在当前事务开启 startTs 之后提交，发生了写冲突
	...
	// be sure that all keys aren't locked
	for _, key := range keys {
...
func (server *Server) KvCheckTxnStatus(_ context.Context, req *kvrpcpb.CheckTxnStatusRequest) (*kvrpcpb.CheckTxnStatusResponse, error) {
	...
	if err != nil {
		// 如果没找到 primarykey write 说明还没有做出决定
		return resp, err
	}
	...
	if err != nil {
		// primarykey write 已经存在 但 lock 为 nil 说明要么已经回滚了要么已经 commit 了
		return resp, nil
	}
